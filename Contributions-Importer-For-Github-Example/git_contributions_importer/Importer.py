...
class Importer:
    ...
    def __init__(self, repos, mock_repo):
        ...
        self.timezone = (60 * 60 * 3)

    def import_repository(self):
        commits_for_last_day = 0

        if self.start_from_last:
            last_committed_date = (self.committer.get_last_commit_date() - self.timezone)
        ...
        for c in self.get_all_commits((last_committed_date+1)):
            print('\nAnalysing commit at ' + time.strftime("%Y-%m-%d %H:%M:%S", time.gmtime(c.committed_date - self.timezone)))
            ...
            committed_date = (c.committed_date - self.timezone)
        ...
        for repo in self.repos:
            for b in repo.branches:
                for c in repo.iter_commits(b.name):
                    if (c.committed_date - self.timezone) < ignore_before_date or (self.ignore_before_date != None and (c.committed_date - self.timezone) < self.ignore_before_date):
                        continue
                    if c.hexsha not in s:
                        s.add(c.hexsha)
                        commits.append(c)
        commits.sort(key=lambda c: (c.committed_date - self.timezone))
        return commits
